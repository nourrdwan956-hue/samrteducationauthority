...
const sendOtpEmailViaSmtpJs = async (toEmail: string, otpCode: string, displayName: string) => {
  const emailHtml = `
...
export const StudentSignUpPage: React.FC = () => {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    fetch('/api/send-otp', {
      ...
      .catch(async (err) => {
        console.warn('Backend SMTP failed, trying client-side SmtpJS fallback...', err);
        try {
          await sendOtpEmailViaSmtpJs(email.trim(), code, fourPartName.trim());
          addToast(
            'success',
            'تم إرسال رمز التأكيد (OTP) ✉️',
            `تم إرسال رمز التحقق بنجاح إلى بريدك الإلكتروني (${email.trim()}). يرجى التحقق من صندوق الوارد أو البريد المهمل (Spam).`
          ...
          console.error('SmtpJS fallback also failed:', fallbackErr);
          addToast(
            'error',
            'فشل إرسال البريد الإلكتروني ⚠️',
            fallbackErr.message || 'يرجى التأكد من صحة البريد الإلكتروني المدخل ومحاولة المحاولة مجدداً.'
          );
        }
      });
  };
  ...
  const handleResendOtp = () => {
    ...
    fetch('/api/send-otp', {
      ...
      .catch(async (err) => {
        console.warn('Backend SMTP resend failed, trying client-side SmtpJS fallback...', err);
        try {
          await sendOtpEmailViaSmtpJs(email.trim(), newCode, fourPartName.trim());
          addToast('success', 'تم إعادة إرسال الرمز 🔄', 'تم إرسال رمز التحقق الجديد إلى بريدك الإلكتروني بنجاح.');
        } catch (fallbackErr: any) {
          console.error('SmtpJS resend fallback also failed:', fallbackErr);
          addToast(
            'error',
            'فشل إعادة الإرسال ⚠️',
