...
const regex = /<h2 className=\{`text-2xl font-black \$\{isLight \? 'text-slate-900' : 'text-white'\}`\}>[\s\S]*?<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">/;

const replacement = `<h2 className={\`text-2xl font-black \${isLight ? 'text-slate-900' : 'text-white'}\`}>
...
const fixed = content.replace(regex, replacement);
