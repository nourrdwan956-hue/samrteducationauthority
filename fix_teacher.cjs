...
const barrier = `  if (!currentUser || (currentUser.role !== 'teacher' && currentUser.role !== 'super_admin')) {
    return (
...
const insertPoint = `  const teacherPlatform =`;
if (!content.includes('صلاحيات غير كافية')) {
  content = content.replace(insertPoint, barrier + '\n' + insertPoint);
  fs.writeFileSync(path, content);
  console.log('Added Teacher barrier');
} else {
  console.log('Teacher barrier already exists');
}
