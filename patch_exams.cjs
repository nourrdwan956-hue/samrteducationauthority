...
const file = path.join(__dirname, 'src/components/teacher/CourseExamsTab.tsx');
let code = fs.readFileSync(file, 'utf8');

// 1. Add `courses` to useApp()
if (!code.includes('    courses,\n  } = useApp();')) {
  code = code.replace(
    '    addToast,\n  } = useApp();',
    '    addToast,\n    courses,\n  } = useApp();'
  );
...
if (!code.includes('const [isDuplicateModalOpen')) {
  code = code.replace(
...
const duplicateFunc = `
  const handleDuplicate = () => {
    if (!duplicateTargetExam || !duplicateTargetCourseId) return;
    const newExam = {
      ...duplicateTargetExam,
      id: crypto.randomUUID(),
      courseId: duplicateTargetCourseId,
      title: duplicateTargetExam.title + ' (نسخة)',
      createdAt: new Date().toISOString(),
      status: 'draft' as const,
      isPublished: false,
      questions: duplicateTargetExam.questions.map(q => ({
        ...q,
        id: crypto.randomUUID(),
        examId: ''
      }))
    ...
    addToast('تم استنساخ الامتحان بنجاح!', 'success');
  };
`;
if (!code.includes('const handleDuplicate = () => {')) {
  code = code.replace(
...
// 4. Add the duplicate icon to the exam card (next to Edit)
const copyButton = `
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    title="نسخ / استنساخ الامتحان"
                  >
                    <Layers className="w-4 h-4" />
                  </button>
`;
if (!code.includes('نسخ / استنساخ الامتحان')) {
  code = code.replace(
      ...
      {isDuplicateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl p-6">
            <h3 className="text-lg font-black mb-4 dark:text-white">استنساخ الامتحان</h3>
            <p className="text-sm text-slate-500 mb-4">اختر الكورس الذي تريد نسخ هذا الامتحان إليه. سيتم إنشاء نسخة جديدة بالكامل دون التأثير على الامتحان الأصلي.</p>
...
`;
if (!code.includes('{/* Modal: Duplicate Exam */}')) {
  code = code.replace(
